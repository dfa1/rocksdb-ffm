...
| `rocksdbffm-tool-ldb`                | `LdbTool` (offline DB inspection/admin) + `java -jar` CLI passthrough to bundled `ldb` |
| `rocksdbffm-tool-sstdump`            | `SstDumpTool` (single SST file inspection) + `java -jar` CLI passthrough to bundled `sst_dump` |
...
| `rocksdbffm-native-windows-x86_64`   | `librocksdb.dll` for Windows x86-64, plus statically-linked `ldb.exe`/`sst_dump.exe` |
| `rocksdbffm-native-windows-aarch64`  | `librocksdb.dll` for Windows aarch64, plus statically-linked `ldb.exe`/`sst_dump.exe` |
...
```kotlin
implementation(platform("io.github.dfa1:rocksdbffm-bom:0.13"))
...
`pkg:maven/io.github.dfa1/rocksdbffm-core@0.13`, which SCA tools (Syft, Grype, Trivy, osv.dev,
...
64 MiB write buffer, 7 levels, compact level 0 at 4 files, slow writes at 20 files, stop writes at
36 files, 64 MiB target file size, 256 MiB base level size, auto-compaction enabled). RocksDB
requires `stop >= slowdown >= compaction trigger` and silently raises whichever value is too low
to satisfy it:
...
`CompactionFilter.create(String name, FilterFn fn)` (`rocksdb_compactionfilter_create`) attaches a
Java callback that decides, per `Put`-originated key-value pair rewritten during compaction, to
...
| Command-line tools         |   ✅    | `rocksdbffm-tool-ldb`/`rocksdbffm-tool-sstdump` — `LdbTool`/`SstDumpTool` wrap the bundled `ldb`/`sst_dump` binaries (not in `c.h`, no C API equivalent) as subprocesses; also directly `java -jar`-able |
| MultiGet                   |   ✅    | `ReadBatch` — the sole entry point for batched reads against a plain `rocksdb_t*` (all three tiers), built on `rocksdb_batched_multi_get_cf` — the modern, PinnableSlice-based, single-CF-per-call variant; reusable and preallocated (create once for up to N keys, no per-call bookkeeping-array allocation on reuse); no separate one-shot `multiGet()` method exists — a single-batch read is just `try (var batch = ReadBatch.create(db, keys.size())) { return batch.get(keys, fn); }`; legacy `rocksdb_multi_get`/`_cf`/`_with_ts` (no batching perf benefit per upstream) are not wrapped. `TransactionDB`/`Transaction` multi-get are separate native symbols returning malloc'd `char*`/`size_t` pairs instead of `rocksdb_pinnableslice_t*` handles, so they get their own reusable batch classes — `ReadBatchTransactionDB` (`rocksdb_transactiondb_multi_get[_cf]`) and `ReadBatchTransaction` (`rocksdb_transaction_multi_get[_for_update][_cf]`, the latter exposed as `getForUpdate`) — sharing collection/free logic via the package-private `RawMultiGet` |
