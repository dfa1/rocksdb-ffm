...
| Shared utilities        | `RocksDB.java` statics (`errHolder`, `checkError`, `toNative`), `NativeObject.java`, `NativeLibrary.java`, `MemorySize.java`, `SequenceNumber.java`, `RocksDBException.java`; `ReadOperations.java`/`ReadColumnFamilyOperations.java`/`WriteOperations.java` — default-method interfaces sharing the direct put/get/merge/delete/iterate/flush/compaction/CF-management surface. `ReadOperations` (no CF methods) is implemented by `SecondaryDB`, which has no multi-CF open path; `ReadColumnFamilyOperations extends ReadOperations` (adds CF-scoped reads) by `ReadOnlyDB`. `WriteOperations` does NOT extend either read interface — `ReadWriteDB`/`TtlDB`/`BlobDB` implement both `ReadColumnFamilyOperations` and `WriteOperations` directly. Not implemented by `TransactionDB`/`OptimisticTransactionDB`, whose direct ops bind their own `MethodHandle`s per the no-shared-call-site rule |
