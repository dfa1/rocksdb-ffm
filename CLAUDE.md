...
- always keep the MethodHandles private static final
    ...
    - **Documented exception:** every `*Options`-style wrapper class extends `AbstractOptions` (not `NativeObject` directly), which provides `protected final` helpers — `getInt`/`setInt`, `getLong`/`setLong`, `getDouble`/`setDouble`, `getBoolean`/`setBoolean`, `getMemorySize`/`setMemorySize` — each taking a `MethodHandle` as a parameter. Plain no-arg getters and single-primitive-argument setters across all option classes (including `ReadOptions`/`WriteOptions`) route through these instead of each inlining its own `try { mh.invokeExact(...) } catch (Throwable t) { ... }`. These are cold-path calls — made once (or once per read/write call in `ReadOptions`/`WriteOptions`, still nowhere near `Get`/`Put`/iterator internals) — so the lost constant-folding is immaterial, and consolidating the near-identical `catch (Throwable t)` blocks meaningfully shrinks the permanently-unreachable defensive code cataloged in ADR 0004. Scope stays tight: only a plain no-arg getter or a setter taking `ptr()` plus exactly one primitive qualifies — a pointer-argument call, a multi-arg call, or anything with branching/validation beyond the value transform (e.g. a `Duration`-sentinel timeout, or a boolean mapped onto a non-0/1 native enum) stays a raw, per-call-site `MethodHandle` invocation. Do not extend this pattern to non-`*Options` classes or any hot-path call site (`ReadWriteDB`, `WriteBatch`, `RocksIterator`, ...) without the same justification
