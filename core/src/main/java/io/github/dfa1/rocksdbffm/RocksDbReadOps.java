...
/// Shared read-only operations for every wrapper around a plain `rocksdb_t*`: read-write,
/// read-only, TTL, blob, and secondary instances all expose the identical read surface.
///
...
public interface RocksDbReadOps {
	...
	/// Returns the [ReadOptions] used when no explicit options are supplied.
	///
	/// @return the default read options
	ReadOptions defaultReadOpts();
