...
/// like [Transaction#getForUpdate(ReadOptions, MemorySegment, MemorySegment, boolean)]'s
/// `_for_update` single-key counterpart. Same
/// reuse shape and malloc'd-buffer value representation as [ReadBatchTransactionDB] — see
/// [RawMultiGet] for the shared collection logic.
///
/// ```
/// try (var batch = ReadBatchTransaction.create(txn, 16)) {
///     List<byte[]> values = batch.getForUpdate(keys);
/// }
/// ```
public final class ReadBatchTransaction implements AutoCloseable {
	...
	private ReadBatchTransaction(Transaction txn, ColumnFamilyHandle cf, int capacity) {
		this.txn = txn;
		this.cf = cf;
	...
	/// @return a new [ReadBatchTransaction]; caller must close it
	public static ReadBatchTransaction create(Transaction txn, int capacity) {
	...
	/// @param capacity maximum number of keys any single call may pass; must be positive
	/// @return a new [ReadBatchTransaction]; caller must close it
	public static ReadBatchTransaction create(Transaction txn, ColumnFamilyHandle cf, int capacity) {
		return new ReadBatchTransaction(txn, cf, capacity);
	}
	...
	public List<byte[]> getForUpdate(List<byte[]> keys) {
		return getForUpdate(NativeCalls.DEFAULT_READ_OPTIONS, keys);
	}
	...
	private List<byte[]> getBytes(ReadOptions readOptions, List<byte[]> keys, boolean forUpdate) {
		...
		try (Arena callArena = Arena.ofConfined()) {
			for (int i = 0; i < n; i++) {
				byte[] key = keys.get(i);
				RawMultiGet.writeKeySlot(bufs.keysArr, bufs.keySizesArr, i, NativeCalls.toNative(callArena, key), key.length);
			}
			RocksDB.requireNoNullEntries(bufs.keysArr, n, "ReadBatchTransaction keys array");
			invoke(readOptions, n, forUpdate);
			return RawMultiGet.collectBytes(bufs.valuesListArr, bufs.valuesListSizesArr, bufs.errsArr, n);
		} catch (Throwable t) {
			throw NativeCalls.wrapInvokeFailure("ReadBatchTransaction.get failed", t);
		}
	}
	...
	public List<CopyResult> getForUpdate(List<ByteBuffer> keys, List<ByteBuffer> values) {
		return getForUpdate(NativeCalls.DEFAULT_READ_OPTIONS, keys, values);
	}
	...
	public <R> List<R> getForUpdate(List<MemorySegment> keys, Mapper<R> fn) {
		return getForUpdate(NativeCalls.DEFAULT_READ_OPTIONS, keys, fn);
	}
	...
	/// [ReadBatchTransactionDB#invoke(ReadOptions, int)]. Each of the four native symbols gets
	...
	private void invoke(ReadOptions readOptions, int n, boolean forUpdate) throws Throwable {
		if (cf == null) {
			...
			if (forUpdate) {
				MH_MULTI_GET_FOR_UPDATE_CF.invokeExact(txn.ptr(), readOptions.ptr(), bufs.cfArr, (long) n,
						bufs.keysArr, bufs.keySizesArr, bufs.valuesListArr, bufs.valuesListSizesArr, bufs.errsArr);
			} else {
				MH_MULTI_GET_CF.invokeExact(txn.ptr(), readOptions.ptr(), bufs.cfArr, (long) n,
						bufs.keysArr, bufs.keySizesArr, bufs.valuesListArr, bufs.valuesListSizesArr, bufs.errsArr);
			}
		}
	}
